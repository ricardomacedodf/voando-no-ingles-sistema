...
const categories = [
  "vocabulário", "expressão", "adjetivo", "verbo", "substantivo",
  "phrasal verb", "advérbio", "preposição", "interjeição", "pronome",
  "conjunção", "idiom", "collocation"
];
    ...
    try {
      ...
      if (item?.id) {
        const updatedItems = currentItems.map((existing) =>
          existing.id === item.id ? { ...existing, ...data } : existing
        );
        saveStoredVocabulary(updatedItems);
      } else {
        saveStoredVocabulary([data, ...currentItems]);
      }
